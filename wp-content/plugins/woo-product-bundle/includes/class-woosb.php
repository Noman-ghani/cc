	...
	class WPCleverWoosb {
		...
		function woosb_row_meta( $links, $file ) {
			if ( plugin_basename( WOOSB_DIR . '/' . basename( WOOSB_FILE ) ) === $file ) {
				$row_meta = array(
					'support' => '<a href="https://wpclever.net/support?utm_source=support&utm_medium=woosb&utm_campaign=wporg" target="_blank">' . esc_html__( 'Premium support', 'woo-product-bundle' ) . '</a>',
				);
		...
		function woosb_cart_item_removed( $cart_item_key, $cart ) {
			if ( isset( $cart->removed_cart_contents[ $cart_item_key ]['woosb_keys'] ) ) {
				...
				foreach ( $woosb_keys as $woosb_key ) {
					$cart->remove_cart_item( $woosb_key );

					if ( ( $woosb_new_key = array_search( $woosb_key, array_column( $cart->cart_contents, 'woosb_key', 'key' ) ) ) !== false ) {
						$cart->remove_cart_item( $woosb_new_key );
					}
				}
			}
		}
		...
		function woosb_add_to_cart_validation( $passed, $product_id ) {
			$woosb_product = wc_get_product( $product_id );

			if ( $woosb_product && $woosb_product->is_type( 'woosb' ) && ( $woosb_ids = get_post_meta( $product_id, 'woosb_ids', true ) ) ) {
				if ( isset( $_POST['woosb_ids'] ) && ( $woosb_product->is_optional() || $woosb_product->has_variables() ) ) {
					$woosb_ids = $_POST['woosb_ids'];
				}

				$woosb_qty = isset( $_POST['quantity'] ) ? (int) $_POST['quantity'] : 1;
				$woosb_ids = WPCleverWoosb_Helper::woosb_clean_ids( $woosb_ids );
		...
		function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
			...
			$has_discount          = ! $fixed_price && ( $discount_amount || $discount_percentage );
			...
			if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
				foreach ( $items as $item ) {
					...
					if ( ! $woosb_item_product || ( $woosb_item_qty <= 0 ) || in_array( $woosb_item_product->get_type(), self::$_types, true ) ) {
						continue;
					}

					$woosb_item_variation_id = 0;
					$woosb_item_variation    = array();
					$woosb_item_price        = WPCleverWoosb_Helper::woosb_get_price( $woosb_item_product, 'min', $has_discount );

					if ( $woosb_item_product instanceof WC_Product_Variation ) {
						// ensure we don't add a variation to the cart directly by variation ID
						$woosb_item_variation_id = $woosb_item_id;
						$woosb_item_id           = $woosb_item_product->get_parent_id();
						$woosb_item_variation    = $woosb_item_product->get_variation_attributes();
					}

					if ( ! $fixed_price && $discount_percentage ) {
						$woosb_item_price *= (float) ( 100 - $discount_percentage ) / 100;
					...
					if ( empty( $woosb_item_key ) ) {
						...
						// save current key
						WC()->cart->cart_contents[ $woosb_item_key ]['woosb_key'] = $woosb_item_key;
			...
			foreach ( $cart_contents as $cart_item_key => $cart_item ) {
				...
				if ( ! empty( $cart_item['woosb_parent_id'] ) ) {
					...
					if ( ! empty( $cart_item['woosb_parent_key'] ) && ! empty( $cart_item['woosb_qty'] ) ) {
						...
						if ( ( $parent_new_key = array_search( $parent_key, array_column( $cart_contents, 'woosb_key', 'key' ) ) ) !== false ) {
				...
				if ( ! empty( $cart_item['woosb_ids'] ) && isset( $cart_item['woosb_fixed_price'] ) && ! $cart_item['woosb_fixed_price'] ) {
					...
					if ( ! empty( $cart_item['woosb_keys'] ) ) {
						...
						foreach ( $cart_item['woosb_keys'] as $woosb_key ) {
							if ( isset( $cart_contents[ $woosb_key ] ) ) {
								$bundle_price += wc_get_price_to_display( $cart_contents[ $woosb_key ]['data'], array(
									'qty'   => $cart_contents[ $woosb_key ]['woosb_qty'],
									'price' => $cart_contents[ $woosb_key ]['woosb_price']
								) );
							}
						}
			...
			if ( 'line_item' === $item->get_type() ) {
				...
				if ( $product && $product->is_type( 'woosb' ) && ( $woosb_items = $product->get_items() ) ) {
					...
					foreach ( $woosb_items as $woosb_item ) {
						...
						if ( ! $woosb_item_product || in_array( $woosb_item_product->get_type(), self::$_types, true ) ) {
							continue;
						}
						...
						if ( $fixed_price ) {
							...
							$woosb_item_price = (float) ( 100 - $discount_percentage ) * WPCleverWoosb_Helper::woosb_get_price( $woosb_item_product, 'min', true ) / 100;
		...
		function woosb_before_order_item_meta( $item_id ) {
			if ( $parent_id = wc_get_order_item_meta( $item_id, '_woosb_parent_id', true ) ) {
		...
		function woosb_order_formatted_line_subtotal( $subtotal, $item ) {
			if ( isset( $item['_woosb_ids'], $item['_woosb_price'] ) ) {
		...
		function woosb_product_data_panels() {
			...
			$woosb_ids_str = WPCleverWoosb_Helper::woosb_clean_ids( $woosb_ids_str );
										...
										if ( ! empty( $woosb_ids_str ) ) {
											...
											if ( is_array( $woosb_items ) && count( $woosb_items ) > 0 ) {
												foreach ( $woosb_items as $woosb_item ) {
													...
													if ( ! $woosb_product || in_array( $woosb_product->get_type(), self::$_types, true ) ) {
														continue;
													}
		...
		function woosb_product_data_li( $product, $qty = 1, $search = false ) {
			...
			$price     = WPCleverWoosb_Helper::woosb_get_price( $product, 'min', false );
			$price_max = WPCleverWoosb_Helper::woosb_get_price( $product, 'max', false );
			...
			echo '<li ' . ( ! $product->is_in_stock() ? 'class="out-of-stock"' : '' ) . ' data-id="' . esc_attr( $product_id ) . '" data-price="' . esc_attr( $price ) . '" data-price-max="' . esc_attr( $price_max ) . '"><span class="move"></span><span class="qty hint--right" aria-label="' . esc_html__( 'Default quantity', 'woo-product-bundle' ) . '">' . $qty_input . '</span> <span class="name">' . $product_name . '</span> <span class="info">' . $product->get_price_html() . '</span> ' . ( $product->is_sold_individually() ? '<span class="info">' . esc_html__( 'sold individually', 'woo-product-bundle' ) . '</span> ' : '' ) . '<span class="type"><a href="' . get_edit_post_link( $product_id ) . '" target="_blank">' . $product->get_type() . ' #' . $product_id . '</a></span> ' . $remove_btn . '</li>';
		}
				...
				if ( ! $product->is_fixed_price() ) {
					...
					if ( $product->is_optional() ) {
						...
						foreach ( $woosb_items as $woosb_item ) {
							...
							if ( $woosb_product ) {
								$prices[] = WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, 1, 'min', $has_discount );
							}
						}
						...
						// min whole
						$min_qty_whole = (float) ( get_post_meta( $product_id, 'woosb_limit_whole_min', true ) ?: 1 );
						...
						// min each
						$min_qty_each = (float) ( get_post_meta( $product_id, 'woosb_limit_each_min', true ) ?: 0 );
						...
						foreach ( $woosb_items as $woosb_item ) {
							...
							if ( $woosb_product ) {
								$min_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, $woosb_item['qty'], 'min', $has_discount );
								$max_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, $woosb_item['qty'], 'max', $has_discount );
							}
						}
						...
						foreach ( $woosb_items as $woosb_item ) {
							...
							if ( $woosb_product ) {
								$woosb_product_price = WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, $woosb_item['qty'], 'min', $has_discount );

								$price += $woosb_product_price;

								if ( $discount_percentage ) {
									// if haven't discount_amount, apply discount percentage
									$price_sale += round( $woosb_product_price * ( 100 - $discount_percentage ) / 100, wc_get_price_decimals() );
								}
							}
						}
		...
		function woosb_coupon_is_valid_for_product( $valid, $product, $coupon, $item ) {
			if ( ( get_option( '_woosb_coupon_restrictions', 'no' ) === 'both' ) && ( isset( $item['woosb_parent_id'] ) || isset( $item['woosb_ids'] ) ) ) {
				// exclude both bundles and bundled products
				return false;
			}

			if ( ( get_option( '_woosb_coupon_restrictions', 'no' ) === 'bundles' ) && isset( $item['woosb_ids'] ) ) {
				// exclude bundles
				return false;
			}

			if ( ( get_option( '_woosb_coupon_restrictions', 'no' ) === 'bundled' ) && isset( $item['woosb_parent_id'] ) ) {
			...
			if ( $woosb_items = $product->get_items() ) {
                ...
                <div class="woosb-products"
					...
					foreach ( $woosb_items as $woosb_item ) {
						...
						if ( ( ! $woosb_product->is_in_stock() || ! $woosb_product->has_enough_stock( $woosb_product_qty ) ) && ( get_option( '_woosb_exclude_unpurchasable', 'no' ) === 'yes' ) ) {
							$woosb_product_qty = 0;
						}
						?>
                        <div class="woosb-product"
                             data-name="<?php echo esc_attr( $woosb_product->get_name() ); ?>"
                             data-id="<?php echo esc_attr( $woosb_product->is_type( 'variable' ) ? 0 : $woosb_item['id'] ); ?>"
                             data-price="<?php echo esc_attr( WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, 1, 'min', $has_discount ) ); ?>"
                             data-qty="<?php echo esc_attr( $woosb_product_qty ); ?>">
							<?php
							do_action( 'woosb_before_item', $woosb_product, $product, $count );
                            ...
                            <div class="woosb-title">
								...
								if ( $woosb_product->is_visible() && ( get_option( '_woosb_bundled_link', 'yes' ) !== 'no' ) ) {
									$woosb_item_name .= '<a ' . ( get_option( '_woosb_bundled_link', 'yes' ) === 'yes_popup' ? 'class="woosq-btn" data-id="' . $woosb_item['id'] . '"' : '' ) . ' href="' . esc_url( $woosb_product->get_permalink() ) . '" ' . ( get_option( '_woosb_bundled_link', 'yes' ) === 'yes_blank' ? 'target="_blank"' : '' ) . '>';
								}
								...
								echo apply_filters( 'woosb_item_name', $woosb_item_name, $woosb_product, $product, $count );
								echo '</div>';
								...
								echo '<div class="woosb-availability">' . wc_get_stock_html( $woosb_product ) . '</div>';
								?>
                            </div>
                                ...
                                <div class="woosb-price">
                                    <div class="woosb-price-ori">
										...
										if ( ! $product->is_fixed_price() && ( $discount_percentage = $product->get_discount() ) ) {
											$show_new_price      = true;
											$woosb_product_price = WPCleverWoosb_Helper::woosb_get_price( $woosb_product, 'min', true ) * ( 100 - $discount_percentage ) / 100;
										} else {
											$show_new_price      = false;
											$woosb_product_price = WPCleverWoosb_Helper::woosb_get_price( $woosb_product, 'min', false );
										}

										switch ( get_option( '_woosb_bundled_price', 'price' ) ) {
											case 'price':
												if ( $show_new_price ) {
													$woosb_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, 1, 'min', $has_discount ), wc_get_price_to_display( $woosb_product, array( 'price' => $woosb_product_price ) ) );
												} else {
											...
											case 'subtotal':
												if ( $show_new_price ) {
													$woosb_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $woosb_product, $woosb_item['qty'], 'min', $has_discount ), wc_get_price_to_display( $woosb_product, array(
														'price' => $woosb_product_price,
														'qty'   => $woosb_item['qty']
													) ) );
												} else {
							...
							do_action( 'woosb_after_item', $woosb_product, $product, $count );
							?>
                        </div>
						<?php
						$count ++;
			...
			if ( ! empty( $woosb_ids ) ) {
				...
				if ( is_array( $woosb_items ) && count( $woosb_items ) > 0 ) {
					foreach ( $woosb_items as $woosb_item ) {
						...
						$woosb_item_qty = (float) ( isset( $woosb_item_arr[1] ) ? $woosb_item_arr[1] : 1 );
						$has_id         = false;
